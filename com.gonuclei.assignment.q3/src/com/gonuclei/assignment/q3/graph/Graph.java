...
final public class Graph {
	...
	public void addDependency(int nodeId1, int nodeId2)
			throws NodeDoesNotExistException, DependeciesAlreadyExistException {
		...
		List<GraphNode> check = getDescendants(nodeId1);
		check.addAll(getAncestors(nodeId1));
		
		for (GraphNode d : check) {
			if (d.getId() == nodeId2)
				throw new DependeciesAlreadyExistException("Dependencies already exist. The Graph will become Cyclic!");
		}
	...
	public void deleteDependency(int nodeId1, int nodeId2) throws NodeDoesNotExistException {
		...
		if (node1 == null || node2 == null)
			throw new NodeDoesNotExistException("Either of a node Doesn't Exist");
	...
	public void deleteNode(int nodeId) throws NodeDoesNotExistException {
		...
		if (node == null) {			
			...
			for (Integer key : nodes.keySet()) {
				...
				if (comingInNodes != null && comingInNodes.contains(node))
					comingInNodes.remove(node);
				tempNode.setComingInNodes(comingInNodes);

				List<GraphNode> goingOutNodes = tempNode.getGoingOutNodes();
				if (goingOutNodes != null && goingOutNodes.contains(node))
					goingOutNodes.remove(node);
